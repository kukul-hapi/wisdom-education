...
public class TestPaperInfoService extends BaseService<TestPaperInfoMapper, TestPaperInfo> {
    ...
    public TestPaperInfoDto getDetail(Integer id) {
        ...
        Long time = Long.valueOf(studentId.toString());
        Integer countDown;
        Integer examTime = testPaperInfo.getExamTime();
        if (time == null) {
            // 缓存开始考试的时间
            boundHashOperations.put(UserSessionContext.getStudentId(), studentId);
            countDown = examTime;
        } else {
        ...
        BeanUtils.copyProperties(testPaperInfoDto, testPaperInfoDto);
